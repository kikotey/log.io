...
function main() {
  case "$action" in
    ...
    debug)
      ...
      ;;
    *)
      @e "Usage: {setup|clean|start|stop|restart|status|debug}"
      exit 1
      ;;
  esac
}
...
@serviceStart() {
  cd $basedir/server && pm2 --name inputs start npm -- start && sleep 5s && cd $basedir/inputs/file && pm2 --name server start npm -- start
  pm2 ps
  pm2 logs
}
...
@serviceClean() {
  ...
  rm -Rf $basedir/ui/node_modules
  rm -Rf $basedir/server/node_modules
  rm -Rf $basedir/inputs/file/node_modules
}
